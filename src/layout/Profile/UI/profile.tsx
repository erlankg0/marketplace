    ...
    return (
        <section className={styles.profile}>
            ...
            <section className={styles.profile__content}>
                <Person image={profile.imagePath} fullName={`${profile.name} ${profile.surname} ${profile.patronymic}`} module={modal} setModal={setModal} />
                <form onSubmit={handleSubmit(onSubmit)}>
                    <div className={styles.form}>
                        <h2 className={styles.form__title}>Личные Данные</h2>
                        <div className={styles.form__row}>
                            <div className={styles.form__field}>
                                <label htmlFor={'name'} className={errors.name ? `${styles.form__label} ${styles.error}` : styles.form__label}>Имя</label>
                                <input placeholder={'Иван'} id={'name'} className={styles.form__input} {...register('name')} />
                            </div>
                            <div className={styles.form__field}>
                                <label htmlFor={'surname'} className={errors.surname ? `${styles.form__label} ${styles.error}` : styles.form__label}>Фамилия</label>
                                <input placeholder={'Иванов'} id={'surname'} className={styles.form__input} {...register('surname')} />
                            </div>
                        </div>
                        <div className={styles.form__field}>
                            <label htmlFor={'patronymic'} className={errors.patronymic ? `${styles.form__label} ${styles.error}` : styles.form__label}>Отчество</label>
                            <input placeholder={'Иванович'} id={'patronymic'} className={styles.form__input} {...register('patronymic')} />
                        </div>
                    </div>
                    <div className={styles.form}>
                        <h2 className={styles.form__title}>Личные Данные</h2>
                        <div className={styles.form__row}>
                            ...
                            <div className={styles.form__field}>
                                ...
                                <input placeholder={'+996 555 555 772'} id={'phoneNumber'} className={styles.form__input} {...register('phoneNumber')} />
                            </div>
                        </div>
                    </div>
            ...
            <Modal active={modal} setModalActive={setModal} component={Alert} componentProps={{ change: true, setModalActive: setModal }} />
        </section>
    );
};
