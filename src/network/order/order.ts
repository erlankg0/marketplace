...
export const getByIdOrder = async (id: number | string, author?: boolean): Promise<IData | IError> => {
    try {
        const url = author ? `order/get-order-detailed-for-author/${id}` : `order/get-order-detailed/${id}`;
        const response: AxiosResponse<IData> = await instance.get(url);
        return response.data;
    } catch (error) {
